/**
 * Trade-tape loader for Tick-Run Bars.
 *
 * Run bars need each execution's price to assign a tick sign, so they are built
 * from a trade tape. Yahoo Finance does not expose tick data — only
 * pre-aggregated bars, where the per-trade signs are already lost.
 */
