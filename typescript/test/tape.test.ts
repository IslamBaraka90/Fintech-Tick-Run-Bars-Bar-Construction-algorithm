...
test("constructBars over the loaded tape", () => {
  ...
  assert.deepEqual(bars.map((b) => b.dominantSide), ["buy", "sell"]);
  assert.deepEqual(bars.map((b) => b.thresholdTicks), [2.5, 2.75]);
  assert.deepEqual(bars.map((b) => b.overshootTicks), [0.5, 0.25]);
});
